...
type ProjectService struct {
	...
	projectTempleRepo      repo.ProjectTemplateRepo
...
func New() *ProjectService {
	return &ProjectService{
		...
		projectTempleRepo:      dao.NewProjectTemplateDao(),
...
func (ps *ProjectService) FindProjectTemplate(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectTemplateResponse, error) {
	...
	var err error
	if msg.ViewType == -1 {
		pts, total, err = ps.projectTempleRepo.FindProjectTemplateAll(ctx, organizationCode, page, pageSize)
	}
	if msg.ViewType == 0 {
		pts, total, err = ps.projectTempleRepo.FindProjectTemplateCustom(ctx, msg.MemberId, organizationCode, page, pageSize)
	}
	if msg.ViewType == 1 {
		pts, total, err = ps.projectTempleRepo.FindProjectTemplateSystem(ctx, page, pageSize)
	}
	if err != nil {
